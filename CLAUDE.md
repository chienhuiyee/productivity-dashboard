...
- `src/lib/github/` — `queries.ts` (aliased batch GraphQL + raw types), `fetchRepos.ts` (chunk + `Promise.allSettled` + defensive parse; `parseRepo`/`runChunk` are pure & unit-tested), `provider.ts` (fetch → rank → cache), `client.ts`, `types.ts`.
- `src/lib/ranking/` — `rules.ts` (all thresholds) + `score.ts` (pure `scorePr`/`scoreAction`/`buildFocus`).
...
## Verify
`npm run typecheck && npm test && npm run build`. Unit tests cover ranking and GraphQL parsing
(null alias, null default branch, bot activity). Full GitHub data flow needs a real OAuth login.
