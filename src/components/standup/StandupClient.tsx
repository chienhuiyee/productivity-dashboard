...
export function StandupClient() {
  const { data, isLoading, mutateOp, action } = useStandup();
  const [sub, setSub] = useState<SubView>("today");

  return (
    <div className="flex flex-col gap-5">
      <div className="inline-flex w-fit items-center gap-0.5 rounded-full border border-border bg-surface p-1">
      ...
      {sub === "today" ? (
        <TodayPanel data={data} isLoading={isLoading} mutateOp={mutateOp} action={action} />
      ) : (
